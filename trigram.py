	...
	for k in j:
		...
		for l in range(length-1):
			if k[l].isalpha():
				if l != length-2:
					if first == False:
						#first word does not have any word before it
						if k[l] in tri[second_prev][prev].keys():
							tri[second_prev][prev][k[l]] += 1
						else:
							tri[second_prev][prev][k[l]] = 1

					if k[l+1].isalpha():
						if k[l+2].isalpha():
							if k[l+2] in tri[k[l]][k[l+1]].keys():
								tri[k[l]][k[l+1]][k[l+2]] += 1
							else:
								tri[k[l]][k[l+1]][k[l+2]] = 1
							las_char = k[l+1]
							second_las = k[l]
				else:
					#previous two letters of the last word to connect to first character of new word
					if k[l+1].isalpha(): 					
						prev = k[l+1]
						second_prev = k[l]
					else:
						if k[l].isalpha():						
							...
							prev = las_char
							second_prev = second_las
		first = False


for a in tri.iterkeys():
	for b in tri[a].iterkeys():
		for alp in Ap:
			if alp not in tri[a][b]:
				tri[a][b][alp] = 1		
			
total = 0
for a in tri.iterkeys():
	for b in tri[a].iterkeys():
		for c in tri[a][b].iterkeys():
			total += tri[a][b][c]

for a in tri.iterkeys():
	for b in tri[a].iterkeys():
		for c in tri[a][b].iterkeys():
			tri[a][b][c] = float(tri[a][b][c])/total	
